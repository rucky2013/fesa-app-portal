...
public class SubStringHTML {
	public static String[]siglehtml=new String[]{"hr","img"};
	/**
	...
	public static String fix(String str) {
		...
		unclosedTags = getUnclosedTags(str);
		//生成新字符串
		...
		for (int i = unclosedTags[1].size() - 1; i > -1; i--) {
			String s = null;Boolean same=false;
			if ((s = unclosedTags[1].get(i)) != null) {
				for(String strtemp : siglehtml) {
					if(s.equals(strtemp)){
						same=true;
					}
				}
	...
	private static TagsList[] getUnclosedTags(String str) {
		...
		for (int i = 0; i < str.length();) {
			...
			if (current == '"' || String.valueOf(current) == "'") {
